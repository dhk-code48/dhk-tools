...
const EditText: FC<{ data: textProps }> = ({ data }) => {
  ...
  function handleEditableState(state: boolean) {
    if (userEditCode === data.attributes.editCode) {
      ...
      toast({
        ...
        variant: "destructive",
        description:
          "Code you have written doesn't match with the original one",
      });
    }
  }
  ...
  const handleContentEdit = async () => {
    ...
    try {
      const res = await fetch(
        `http://192.168.18.87:1337/api/rich-texts/${data.id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        ...
        toast({
          ...
          variant: "destructive",
          description:
            "There was an error while editing text, please try again !!",
        });
      }
    } catch (error) {
      toast({
  ...
  async function handleVal() {
    try {
      const isValidPassword = await handleValidation(
        password,
        ...
        <>
          ...
          {!editable && (
            <div className="flex gap-4 items-center">
              ...
              <Switch
                checked={editable}
                onCheckedChange={handleEditableState}
              />
            </div>
          )}
        </>
