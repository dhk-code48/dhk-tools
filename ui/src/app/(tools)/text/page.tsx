...
const WriteText: FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("http://localhost:1337/api/rich-texts", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        ...
        toast({
          ...
          variant: "destructive",
          description:
            "There was an error while adding text, please try again !!",
        });
      }
    } catch (error) {
      toast({
    ...
    <div className="items-center space-y-5">
      ...
      <div className="flex items-start gap-10">
        <div>
          ...
          <label
            className={`${
              customUrl.length < 5 || customUrl.length > 20
                ? "text-red-500"
                : "text-green-500"
            } text-sm py-3 pl-1`}
          >
        ...
        <div>
          ...
          <label
            className={`${
              editCode.length < 3 || editCode.length > 20
                ? "text-red-500"
                : "text-green-500"
            } text-sm py-3 pl-1`}
          >
